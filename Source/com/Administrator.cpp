...
namespace RPC {
    ...
    bool Administrator::StubFind(const Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<InvokeMessage>& message)
    {
        uint32_t interfaceId(message->Parameters().InterfaceId());
        instance_id implementation(message->Parameters().Implementation());

        ReferenceMap::iterator crmindex = _channelReferenceMap.find(channel.operator->());
        if (crmindex != _channelReferenceMap.end()) {
            StubList::iterator element = std::find_if(crmindex->second.begin(), crmindex->second.end(),
                    [interfaceId, implementation] (StubList::value_type &element) -> bool {
                        return ((element.first == interfaceId) && (element.second == reinterpret_cast<Core::IUnknown*>(implementation)));
                    });

            return (element != crmindex->second.end());
        }
    ...
    void Administrator::Invoke(Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<InvokeMessage>& message)
    {
        ...
        if (index != _stubs.end()) {
            if(StubFind(channel, message) || (interfaceId == RPC::ID_SHELL)) {
                index->second->Handle(methodId, channel, message);
            } else {
                TRACE_L1("Unknown interface. %d(%p)", interfaceId, implementation);
            }
        } else {
            // Oops this is an unknown interface, Do not think this could happen.
            TRACE_L1("Unknown interface. %d(%p)", interfaceId, implementation);
    ...
    void Administrator::DeleteChannel(const Core::ProxyType<Core::IPCChannel>& channel, std::list<ProxyStub::UnknownProxy*>& pendingProxies)
    {
        ...
        if (remotes != _channelReferenceMap.end()) {
            StubList::iterator loop(remotes->second.begin());
